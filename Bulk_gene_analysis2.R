...
vlepo.enrich=function(trial,omic){
  ...
  enriched= combined %>% arrange(fdr) %>% filter(fdr<1e-02) %>% filter(test_N>3) %>% select(-frequency,-code_count) %>% 
...
files=c(gut.specific.genes,comp.de.files,comp.fuzz.files,top300.files)
short=c()
